...
import { Plus, Edit, Shield, AlertTriangle, Users, Phone, CalendarDays, Star } from 'lucide-react';
...
export default function DriversPage() {
    ...
    const handleSave = async (e) => {
        ...
        try {
            if (editing) {
                await apiCall(`/drivers/${editing._id}`, 'PATCH', data);
            } else {
                await apiCall('/drivers', 'POST', { ...data, status: 'On Duty', tripsCompleted: 0, joinDate: today });
            }
            await refreshData();
            setModalOpen(false);
        } catch (err) {
    ...
    const updateStatus = async (id, status) => {
        try {
            await apiCall(`/drivers/${id}`, 'PATCH', { status });
            await refreshData();
        } catch (err) {
            alert(err.message);
        }
    ...
    const getCompletionRate = (driverId) => {
        const driverTrips = trips.filter(t => (t.driverId?._id || t.driverId) === driverId);
    ...
    const columns = [
        ...
        },
        {
            key: 'completion', label: 'Completion', accessor: row => row.tripsCompleted, render: row => (
                <span>{row.tripsCompleted} trips ({getCompletionRate(row._id)})</span>
            )
        },
        { key: 'status', label: 'Status', accessor: 'status', render: row => <StatusPill status={row.status} /> },
        {
            key: 'actions', label: 'Actions', sortable: false, hidden: !canManage, render: row => (
                <div className="action-btns">
                    ...
                    {row.status !== 'On Trip' && (
                        <>
                            {row.status !== 'On Duty' && <button className="btn-icon" onClick={() => updateStatus(row._id, 'On Duty')} title="Set On Duty"><Shield size={15} /></button>}
                            {row.status !== 'Off Duty' && <button className="btn-icon" onClick={() => updateStatus(row._id, 'Off Duty')} title="Set Off Duty" style={{ color: 'var(--text-muted)' }}><Shield size={15} /></button>}
                            {row.status !== 'Suspended' && <button className="btn-icon btn-danger" onClick={() => updateStatus(row._id, 'Suspended')} title="Suspend"><AlertTriangle size={15} /></button>}
                        </>
                    )}
                </div>
            )
    ...
    return (
        <div className="page-content">
            <div className="page-header">
                <div>
                    <h1 style={{ display: 'flex', alignItems: 'center', gap: '10px' }}><Users size={22} style={{ color: 'var(--accent)' }} />Driver Profiles</h1>
            ...
            {expiringDrivers.length > 0 && (
                <div className="info-card" style={{ background: 'rgba(239,68,68,0.05)', borderColor: 'rgba(239,68,68,0.2)', marginBottom: '24px' }}>
                    <p style={{ margin: 0, fontSize: '0.85rem', color: 'var(--text-muted)' }}>
                        ...
                        <div style={{ marginTop: '8px', display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                            {expiringDrivers.map(d => (
                                <span key={d._id} style={{ fontSize: '0.75rem', padding: '4px 10px', background: 'rgba(239,68,68,0.1)', color: '#ef4444', borderRadius: '6px', border: '1px solid rgba(239,68,68,0.2)' }}>
                                    {d.name} ({new Date(d.licenseExpiry).toLocaleDateString()})
                                </span>
                            ))}
                        </div>
                    </p>
            ...
            <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit Driver' : 'Add Driver'}>
                <form onSubmit={handleSave} className="modal-form">
                    <div className="form-row">
                        <div className="form-group"><label style={{ display: 'flex', alignItems: 'center', gap: '6px' }}><Users size={13} />Name</label><input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} required /></div>
                        <div className="form-group"><label style={{ display: 'flex', alignItems: 'center', gap: '6px' }}><Phone size={13} />Phone</label><input value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value })} required /></div>
                    </div>
                    ...
                    <div className="form-row">
                        <div className="form-group"><label style={{ display: 'flex', alignItems: 'center', gap: '6px' }}><CalendarDays size={13} />License Expiry</label><input type="date" value={form.licenseExpiry} onChange={e => setForm({ ...form, licenseExpiry: e.target.value })} required /></div>
                        <div className="form-group"><label style={{ display: 'flex', alignItems: 'center', gap: '6px' }}><Star size={13} />Safety Score (0–100)</label><input type="number" min="0" max="100" value={form.safetyScore} onChange={e => setForm({ ...form, safetyScore: e.target.value })} /></div>
                    </div>
                    <div className="form-actions">
                        <button type="button" className="btn-secondary" onClick={() => setModalOpen(false)}>Cancel</button>
                        <button type="submit" className="btn-primary" disabled={isSaving}>
                            {isSaving ? 'Processing...' : (editing ? 'Save' : 'Add Driver')}
                        </button>
                    </div>
                </form>
            </Modal>
